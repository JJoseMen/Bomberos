...
export function CertificadosEmitidosPage() {
  ...
  const fetchData = async (p: number, searchVal: string) => {
    ...
    try {
      ...
      const res = await profesionalesService.listarCertificados(params);
  ...
  const handleDescargar = async (codigo: string) => {
    try {
      const response = await api.get(`/admin/profesionales/certificados/${codigo}/descargar`, {
        responseType: 'blob',
      });
  ...
  const cols: ColumnDef<CertificadoProfesional, unknown>[] = useMemo(
    () => [
      ...
      {
        header: 'CI / NIT',
        cell: ({ row }) => <span>{row.original.solicitud.empresa?.nit || '-'}</span>,
      },
      {
        header: 'Emisión',
        ...
        cell: ({ row }) => (
          <div className={styles.actions}>
            <Button
              ...
              onClick={() =>
                navigate(
                  `/admin/profesionales/solicitudes/${row.original.solicitud.subtipoTramite.toLowerCase()}/${row.original.solicitud.codigoFormulario}`,
                )
              }
            >
              Ver
  ...
  return (
    <div className={styles.page}>
      <SectionTitle
        breadcrumb={['Admin', 'Profesionales', 'Certificados Emitidos']}
        titulo="Certificados Emitidos"
        subtitulo="Todos los certificados emitidos (documentos PDF con QR)"
      />
